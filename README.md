...
タスクを長押しすると、そのまま指を動かして並べ替えられます。
動かさずに離すと、削除の確認が出ます。

並べ替えると並び順が「手動」に切り替わります。
絞り込みのバーの右にある並び順のボタンで、期限順・優先度順に戻せます。

## 右下の入り口

右下のボタンを押すと「買い物」と「メモ」が出ます。

### 買い物

その日に買うものを書いておく場所です。チェックボックスと個数が付きます。
書いた順のまま並ぶので、売り場を回る順に書いておけます。
買ったものはまとめて消せます。常に 1 つで、上限は 60 件です。

## メモ

右下のボタンから、どこにも属さない 1 枚のメモを開けます。
タスクにするほどでもないもの（買い物の走り書き、電話の要点、下書き）の置き場所です。

常に 1 枚だけで、増やせません。上限は 500 文字です。
打つたびに保存され、ログインしていれば端末をまたいで同期します。
...
| サブタスク | 詳細画面で追加。**一覧にもチェックボックス付きで並ぶ**ので、開かずに片付けられる。1 件ずつ期限を付けられ（親の期限より後は選べません）、過ぎたものは親と同じように強調されます。チェックした数から進捗が自動計算される |
| サブタスクの並べ替え | 詳細画面で、行の左端の**取っ手（☰）を長押し**してつかみ、動かして落とす |
| サブタスクを消したとき | 下に「元に戻す」が出ます。押すと**元の位置に**戻ります |
...
通知は**期限の時刻ちょうど**に出ます（閉じている間の push はサーバー側の巡回が
毎分なので、最大 1 分ずれます）。圏外などで遅れて届いたぶんは、定刻の通知と
区別できるよう「期限を過ぎています」という言い方になります。
